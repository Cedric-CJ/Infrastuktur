...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { content_id, text } = JSON.parse(event.body || "{}");
    if (!content_id || !text) return response(400, { error: "content_id & text required" });
    ...
    await db.send(
      new PutItemCommand({
        ...
        Item: {
          ...
          msg: { S: `comment by ${user_id}` },
          ttl: { N: `${Math.floor(Date.now() / 1000) + 7 * 24 * 3600}` },
        },
      })
    );

    return response(201, { ok: true, comment_id });
  } catch (err) {
    console.error(err);
